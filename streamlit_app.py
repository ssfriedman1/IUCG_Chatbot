...
st.set_page_config(page_title="Chat with IUCG consulting assistant!", page_icon="🦙", layout="centered", initial_sidebar_state="auto", menu_items=None)
openai.api_key = st.secrets.openai_key
st.title("Chat with the IUCG Consulting Assistant 💬🦙")
    ...
    try:
        with st.spinner(text="Loading and indexing the IUCG Intranet – hang tight! This should take 1-2 minutes."):
            ...
            for file_path in reader.get_all_files():
                try:
                    doc = reader.load_document(file_path)
                    if doc:
                        docs.append(doc)
